...
start "P-Agent Backend" cmd /c "cd /d "%~dp0backend" && uv run uvicorn app.main:app --host 127.0.0.1 --port 8756"

REM 백엔드가 준비될 시간을 준다.
timeout /t 5 /nobreak >nul

if exist frontend\node_modules (
    start "P-Agent" cmd /c "cd /d "%~dp0frontend" && pnpm tauri dev"
) else (
