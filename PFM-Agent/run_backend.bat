@echo off
REM ============================================================
REM  PFM-Agent 백엔드 단독 실행 (FastAPI + LangGraph)
...
echo PFM-Agent 백엔드를 시작합니다...
echo   API 문서: http://127.0.0.1:8756/docs
echo.
cd backend
"%~dp0backend\.venv\Scripts\python.exe" -m uvicorn app.main:app --host 127.0.0.1 --port 8756 --reload
pause
