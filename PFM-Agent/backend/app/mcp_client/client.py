...
_logger = logging.getLogger("pfm-agent.mcp.client")
...
class MCPClient:
    ...
    async def connect(self) -> None:
        ...
        ⚡ 서버끼리 의존 관계가 없으므로 **동시에** 기동한다.
        순차 기동은 (서버 1개 기동 시간 × 서버 수) 만큼 걸려 앱 시작이 느려진다.
        ...
        targets = [(name, cfg) for name, cfg in self.configs.items() if cfg.enabled]
        if not targets:
            return

        connections = [_ServerConnection(cfg, self.timeout) for _, cfg in targets]
        ...
        try:
            ...
            # 기동 도중 취소되면 이미 떠 있는 프로세스를 정리한다. (좀비 프로세스 방지)
        ...
        # 설정 파일 순서대로 등록해 결과가 항상 같은 순서로 보이게 한다.
        for (name, _), connection, result in zip(
            targets, connections, results, strict=True
        ):
            if isinstance(result, BaseException):
                self._failed[name] = str(result)
                continue
    ...
    async def aclose(self) -> None:
        ...
        ⚡ 종료도 동시에 수행한다. (순차 종료는 서버 수만큼 대기 시간이 누적된다)
        """
...
async def _run_healthcheck(config_path: Path | None = None) -> int:
    ...
    print("=" * 56)
    print("  PFM-Agent 내부 도구 서버(MCP) 헬스체크")
    print("=" * 56)
...
def main() -> None:
    """CLI 진입점."""
    parser = argparse.ArgumentParser(description="PFM-Agent MCP 클라이언트")
    parser.add_argument(
        "--healthcheck",
        action="store_true",
