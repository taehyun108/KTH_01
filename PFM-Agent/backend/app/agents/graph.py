...
def get_graph(runtime: AgentRuntime) -> Any:
    """
    런타임에 컴파일된 그래프를 반환한다. (없으면 만들어 캐시)

    ⚡ 그래프 구조는 런타임마다 고정이므로 실행마다 다시 컴파일할 이유가 없다.
    요청이 잦은 회사 PC 에서 매 실행 컴파일 비용을 없애기 위한 캐시다.
    """
    if runtime.compiled_graph is None:
        runtime.compiled_graph = build_graph(runtime)
    return runtime.compiled_graph
