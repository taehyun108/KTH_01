...
from app.agents.runtime import MAX_TRACKED_RUNS, AgentRuntime, current_run_id
from app.agents.state import AgentState, initial_state

logger = logging.getLogger("pfm-agent.run_manager")
...
class RunManager:
    ...
    ⚡ 보관 실행 수에 상한을 둔다. 앱을 켜 둔 채 요청을 계속 넣으면
    기록이 무한히 쌓여 메모리를 잠식하기 때문이다.
    상한을 넘으면 **이미 끝난** 실행 중 가장 오래된 것부터 버린다.
    (진행 중인 실행은 절대 버리지 않는다)
    """

    def __init__(
        self, runtime: AgentRuntime, *, max_runs: int = MAX_TRACKED_RUNS
    ) -> None:
    ...
    def _evict_old_runs(self) -> None:
        ...
        while len(self._runs) > self._max_runs:
            ...
            if evictable is None:
                return  # 전부 진행 중이면 아무것도 버리지 않는다.
            self._runs.pop(evictable, None)
            # 이벤트 기록도 함께 정리해 메모리를 돌려준다.
            self.runtime.event_bus.clear(evictable)
