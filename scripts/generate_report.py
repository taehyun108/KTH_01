...
· 성인 독자를 위한 <차분한 설명체(‘~합니다/~입니다’)>로 쓴다.
  아이에게 말하듯 하는 ‘~해요’체나 과하게 귀여운 표현은 쓰지 않는다.
  동시에 ‘~이다/~한다’ 식의 논문·보고서 문어체도 쓰지 않는다.
  예) (X) "관세 부과가 밸류체인에 부정적 영향을 미칠 것으로 분석된다."   ← 너무 딱딱함
      (X) "관세가 붙으면 배터리 만드는 비용이 올라서 가격도 오를 수 있어요."  ← 너무 어림
      (O) "관세가 붙으면 배터리 제조 원가가 올라, 결국 판매 가격에도 반영될 수 있습니다."
· 쉬운 것과 유치한 것은 다르다. 어휘는 평이하게 쓰되, 용어는 정확한 말을 쓰고
  바로 뒤에 풀이를 붙인다. 억지 비유나 의성어·의태어(쌩쌩, 폭삭 등)는 쓰지 않는다.
· 어려운 용어·영어 약어는 처음 나올 때 괄호로 바로 풀어 준다.
  예) "FOMC(미국의 기준금리를 결정하는 회의)", "CapEx(설비·공장에 들어가는 대규모 투자)",
      "밸류체인(원료에서 부품을 거쳐 완성품에 이르는 사슬)".
· 문장은 짧고 명확하게. 한 문장에 한 가지 내용만 담는다.
· 숫자는 나열만 하지 말고 규모가 가늠되도록 설명한다. 예) "약 3조 원(중견기업 연매출 수준)".
· 늘 '그래서 이것이 배터리 산업에 어떤 의미인지'에 답한다. 용어 나열이 아니라 이해가 목표다.
· 과장이나 억측은 하지 않는다. 사실만 전달하되, 표현은 평이하게 한다.
...
JSON_SPEC = """반드시 아래 구조의 JSON '하나'만 출력하라. 다른 텍스트/마크다운은 금지한다.
{
  ...
  "meta_description": "한줄요약(카드·목록용, 평이한 설명체 ~합니다)",
  ...
  "summary": "02 핵심 내용 구조 (2~3문장, 평이한 설명체)",
  "sections": [{"heading": "소제목", "body": "3~6문장, 평이한 설명체로 풀어서"}, ... 3~4개],
  "battery_implication": "이차전지 산업 시사점 본문만 (공급/ESS/EV/AIDC 축 최소 1개, 평이한 설명체, '07' 같은 머리말 없이)",
  "glossary": [{"term": "용어", "desc": "한줄 설명(배경지식 없이도 이해되게)", "analogy": "비유·예시"}, ... 3개]
}
모든 서술형 필드는 차분한 설명체(‘~합니다/~입니다’)로, 배경지식 없는 성인이 이해할 수 있게 작성한다.
...
[말투·표현 원칙]
· 성인 독자를 위한 차분한 설명체(‘~합니다/~입니다’). 아이에게 말하듯 하는 ‘~해요’체나
  과하게 귀여운 표현은 쓰지 않는다. 논문·보고서식 문어체(‘~이다/~한다’)도 쓰지 않는다.
· 쉬운 것과 유치한 것은 다르다. 용어는 정확히 쓰되 바로 뒤에 풀이를 붙인다.
  억지 비유나 의성어·의태어는 쓰지 않는다.
· 문장은 짧고 명확하게, 한 문장에 한 가지.
· 숫자는 규모가 가늠되도록 설명한다. 예) "약 3조 원(중견기업 연매출 수준)".
· 늘 '그래서 이것이 어떤 의미인지'에 답한다.
...
JSON_SPEC_GENERAL = JSON_SPEC.replace(
    ...
    '"battery_implication": "이차전지 산업 시사점 본문만 (공급/ESS/EV/AIDC 축 최소 1개, 평이한 설명체, \'07\' 같은 머리말 없이)",',
    '"battery_implication": "산업·시장 시사점 본문만 (이 영상이 어떤 산업에 어떤 의미인지, 평이한 설명체, \'07\' 같은 머리말 없이)",',
)
